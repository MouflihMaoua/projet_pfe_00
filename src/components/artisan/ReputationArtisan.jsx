...
// ════════════════════════════════════════════════════════════════
//  SYSTÈME RÉPUTATION ARTISAN
//  ─────────────────────────────────────────────────────────────
//  • Score global calculé depuis tous les avis clients
//  • Badge dynamique selon le score (Nouveau / Fiable / Expert / Élite)
//  • Évolution du score dans le temps (↑↓)
//  • Réponse artisan à chaque avis
//  • Vue publique (particuliers) + vue privée (artisan dashboard)
//  • Filtres par étoiles, par service, par date
//  • Classement impact sur la visibilité dans les recherches
// ════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════
export default function ReputationArtisan() {
  const [view, setView] = useState("artisan"); // "artisan" | "public"
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500 font-mono">Système de réputation</div>
            </div>
          </div>
          <div className="flex gap-1 bg-gray-100 p-1 rounded-lg border border-gray-200">
            {[
              {id:"artisan", label:"👷 Vue Artisan"},
              {id:"public",  label:"🌐 Vue Publique"},
            ].map(v=>(
              <button key={v.id} onClick={()=>setView(v.id)} className={`
                px-4 py-2 rounded-md text-xs font-semibold cursor-pointer transition-all duration-200
                ${view===v.id 
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
            ...
            {filtered.map((avis,i)=>(
              ...
              `}>
                <div className="p-5">
                  ...
                  {/* Bouton répondre (vue artisan seulement) */}
                  {view==="artisan"&&!replies[avis.id]&&(
                    replyOpen===avis.id ? (
        ...
        {/* ── IMPACT CLASSEMENT (vue artisan uniquement) ── */}
        {view==="artisan"&&(
